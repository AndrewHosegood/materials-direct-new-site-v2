jQuery(document).ready(function($) {
    ...
    if (!isProductSingle) {
        ...
        $('#generate_price').on('click', function() {
            ...
            const sheet_width_mm = parseFloat($('#custom-price-calc').data('sheet-width-mm'));
            const sheet_length_mm = parseFloat($('#custom-price-calc').data('sheet-length-mm'));
            const border = 2;
            ...
            if (v1 > sheet_width_mm || v2 > sheet_length_mm) {
                $('#custom_price_display').html('Error: Part dimensions (including 2mm border) exceed sheet size (' + sheet_width_mm + 'mm x ' + sheet_length_mm + 'mm) and cannot be manufactured.');
                return;
            }

            // Calculate parts per sheet to ensure at least one part fits
            const parts_per_row = Math.floor(sheet_width_mm / v1);
            const parts_per_column = Math.floor(sheet_length_mm / v2);
        ...
        $('.woocommerce-cart-form, form.cart').on('submit', function(e) {
            ...
            // Additional validation for part dimensions
            const width = parseFloat($('#input_width').val());
            const length = parseFloat($('#input_length').val());
            const sheet_width_mm = parseFloat($('#custom-price-calc').data('sheet-width-mm'));
            const sheet_length_mm = parseFloat($('#custom-price-calc').data('sheet-length-mm'));
