            ...
            $.ajax({
                ...
                success: function(response) {
                    ...
                    if (response.success) {
                        ...
                        const partsPerSheet = response.data.parts_per_sheet || 1;
                        ...
                        // Handle backorder message
                        var backorderMessageContainer = $('.product-page__backorder-message');
                        if (sheetsRequired > stockQuantity) {
                            // Calculate backordered sheets and parts
                            const backorderedSheets = sheetsRequired - stockQuantity;
                            const backorderedParts = backorderedSheets * partsPerSheet;
                            const inStockParts = stockQuantity * partsPerSheet;
                            const message = `<strong>Notice:</strong> This order exceeds current stock, it requires an additional ${backorderedSheets} sheet(s) (${backorderedParts} part(s)) to be back ordered. We are able to despatch: ${inStockParts} parts within 24Hrs (working day). Please allow 35 Days to complete the order. A 5% discount will apply to these parts.`;

                            // Create or update message
                            if (backorderMessageContainer.length === 0) {
                                $('#custom-price-calc').prepend(
                                    '<div class="product-page__backorder-message">' +
                                    '<p class="product-page__backorder-message-text">' + message + '</p>' +
                                    '</div>'
                                );
                            } else {
                                backorderMessageContainer.find('.product-page__backorder-message-text').html(message);
                            }
                        } else {
                            // Remove message if it exists
                            backorderMessageContainer.remove();
                        }
        ...
        $.ajax({
            ...
            success: function(response) {
                if (response.success) {
                    ...
                    // Handle backorder message for single products (unlikely but included for completeness)
                    var backorderMessageContainer = $('.product-page__backorder-message');
                    const sheetsRequired = response.data.sheets_required || 1;
                    const stockQuantity = response.data.stock_quantity || 0;
                    ...
                    if (sheetsRequired > stockQuantity) {
                        const message = `<strong>Notice:</strong> This order exceeds current stock, it requires an additional ${sheetsRequired - stockQuantity} sheet(s) (1 part(s)) to be back ordered. We are able to despatch: ${stockQuantity} parts within 24Hrs (working day). Please allow 35 Days to complete the order. A 5% discount will apply to these parts.`;
                        if (backorderMessageContainer.length === 0) {
                            $('#custom-price-calc').prepend(
                                '<div class="product-page__backorder-message">' +
                                '<p class="product-page__backorder-message-text">' + message + '</p>' +
