...
function custom_price_input_fields_prefill() {
    ...
    echo '<div class="product-page__stages-heading"><h3 class="product-page__stages-heading-content one">Tell us what to manufacture and give us your delivery date</h3></div>
        ...
        <div id="choose_inches_radius" class="product-page__input-wrap unstyled centered" style="width: 100%;">
            ...
            <label for="use_inches_radius">Choose Inches (Diameter)</label>
        </div>

        <label id="cont_radius_inches" class="product-page__input-wrap-radius">Diameter (INCHES): <input class="product-page__input" type="number" id="input_radius_inches" name="custom_radius_inches" min="0.01" step="any"></label> 
        <label class="product-page__input-wrap-radius">Diameter (MM): <input class="product-page__input" type="number" id="input_radius" name="custom_radius" min="0.01" step="any"></label>
...
add_action('wp_enqueue_scripts', function() {
    if (is_product()) {
        ...
        wp_localize_script('custom-price-calc', 'ajax_params', [
            ...
            'diagnostic_url'=> rest_url('materials-direct/v1/price-error'),
...
function add_custom_price_cart_item_data_secure($cart_item_data, $product_id) {
    ...
    // tesday 27th August fix to make capture cart orders behave
    if (!isset($cart_item_data['custom_inputs']) || !is_array($cart_item_data['custom_inputs'])) {
    ...
    /* Determine whether the current user has a credit account */
    $user_id = get_current_user_id();
    $credit_options = get_field('credit_options', 'user_' . $user_id);
    $allow_credit = $credit_options['allow_user_credit_option'] ?? false;
    ...
    if ($is_product_single) {
        ...
        // we need to force all single products to go into the calendar
        $cart_item_data['custom_inputs']['is_scheduled'] = true; // lets mark this as a scheduled order to force it into the calendar

        // lets retrieve the single product quantity
        $single_qty = isset($_POST['quantity']) ? max(1, intval($_POST['quantity'])): 1;
        
        // calculate the single product shipping
        $final_shipping = calculate_shipping_cost($total_del_weight, $country);

        // Determine despatch date for single products 

        $stock_quantity = $product->get_stock_quantity();
        $is_backorder = ($stock_quantity !== null && $stock_quantity <= 0);
        ...
        if ($is_backorder) {
            ...
            $despatch_notes =
                'Single product to be despatched in 35 Days (working days) (5% discount)';

        } else {

            $discount_rate = 0;

            $shipments = date('d/m/Y', strtotime('+1 day'));

            $despatch_notes =
                'Single product to be despatched in 24Hrs (working day)';

        }

        // Generate despatch string only for logged-in credit customers 

        $despatch_string = '';

        if ( is_user_logged_in() && $allow_credit ) {

            $despatch_string = sprintf(
                '%d, %s, %s, ',
                $single_qty,
                $shipments,
                $discount_rate
        ...
        error_log("Despatch String");
        error_log($despatch_string );
        ...
        $cart_item_data['custom_inputs'] = array_merge($cart_item_data['custom_inputs'], [
            ...
            'despatch_string' => $despatch_string,
            'shipments' => $shipments,
            'total_del_weight' => $total_del_weight,
    ...
    error_log('========== NORMAL CART custom_inputs ==========');
    error_log(print_r($cart_item_data['custom_inputs'], true));
    error_log('========== END NORMAL CART custom_inputs ==========');
    ...
    if (!empty($cart_item['custom_inputs'])) {
        ...
        if (
            ...
            if (isset($cart_item['custom_inputs']['width']) && (float) $cart_item['custom_inputs']['width'] > 0) {
                ...
                $diameter_value = (float) $cart_item['custom_inputs']['width'];
        ...
        // Width
        if (isset($cart_item['custom_inputs']['width']) && $cart_item['custom_inputs']['shape_type'] !== 'circle-radius') {
            $item_data[] = [
                'name'  => 'Width (MM)',
        ...
        // Width Inches
        if (isset($cart_item['custom_inputs']['width_inches']) && (float) $cart_item['custom_inputs']['width_inches'] > 0 && $cart_item['custom_inputs']['shape_type'] !== 'circle-radius') {
            $item_data[] = [
                'name'  => 'Width (INCHES)',
        ...
        // Length
        if ($cart_item['custom_inputs']['shape_type'] !== "rolls" && $cart_item['custom_inputs']['shape_type'] !== 'circle-radius') {
        ...
        // Length Inches
        if (isset($cart_item['custom_inputs']['length_inches']) && (float) $cart_item['custom_inputs']['length_inches'] > 0 && $cart_item['custom_inputs']['shape_type'] !== 'circle-radius') {
            $item_data[] = [
                'name'  => 'Length (INCHES)',
