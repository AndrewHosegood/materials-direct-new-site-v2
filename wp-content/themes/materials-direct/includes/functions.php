...
require_once('includes/cofc-logic-for-cart-page-new.php');
/* Add logic for calculating COFC fees on cart page */
...
require_once('includes/meta-labels-admin-orders-and-thankyou-page-new.php');
// style up meta labels on thankyou page and admin orders page
...
// Custom Ivory Search
require_once('includes/custom-ivory-search-v3.php');
// Custom Ivory Search
...
// Remove weights from invoice PDF that is generated by plugin
require_once('includes/remove-weights-from-invoice-pdf-plugin.php');
// Remove weights from invoice PDF that is generated by plugin

// change the shipping label on the cart and checkout page
require_once('includes/change-shipping-label-on-checkout-page.php');
// change the shipping label on the cart and checkout page

// Change the cross sells heading on the cart and checkout page
require_once('includes/change-cross-sells-heading-on-cart-page.php');
// Change the cross sells heading on the cart and checkout page

// Custom oversize surcharge notice on cart page
require_once('includes/custom_oversize_surcharge_notice.php');
// Custom oversize surcharge notice on cart page
...
add_filter( 'wpo_wcpdf_hidden_order_itemmeta', function( $hidden_meta ) {

    $hidden_meta[] = 'cost_per_part';
    $hidden_meta[] = 'Cost Per Part';
...
// Temp function to clean up capture carts
// run http://localhost:8888/?run_capture_cleanup

/*
add_action('init', function() {

    if (current_user_can('administrator') && isset($_GET['run_capture_cleanup'])) {

        cleanup_expired_capture_carts();

        die('Capture cart cleanup executed.');
...
/*
add_action('woocommerce_checkout_order_processed', 'save_session_shipping_to_user_profile', 10, 3);

function save_session_shipping_to_user_profile($order_id, $posted_data, $order) {
    ...
    $shipping = WC()->session->get('custom_shipping_address');
    ...
    $country_code = sanitize_text_field($shipping['country']);

    update_user_meta($user_id, 'shipping_address_1', sanitize_text_field($shipping['street_address']));
    update_user_meta($user_id, 'shipping_address_2', sanitize_text_field($shipping['address_line2']));
