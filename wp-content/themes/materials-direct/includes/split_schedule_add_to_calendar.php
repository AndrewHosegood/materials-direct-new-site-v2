...
function custom_debug_entire_order($order_id) {
    ...
    foreach ($order_items as $item_id => $item) {
        ...
        foreach ($matches as $match) {
            ...
            echo '<pre style="color:red; background:#fff; border:1px solid #ccc; padding:15px; margin:10px 0;">';
            echo "Inserting delivery schedule:\n";
            print_r($data);
            echo '</pre>';
    ...
    if ($has_inserted) {
        echo "<p style='color:green; font-weight:bold; font-size:16px; text-align:center; margin:20px 0;'>Delivery Options Order Completed Successfully</p>";
    }
}
