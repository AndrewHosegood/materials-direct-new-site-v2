...
function add_voucher_field_to_checkout() {
    ...
    <?php if(empty($voucher_display)){ ?>
        <tr class="voucher-discount-toggle">
            ...
            <td class="voucher-discount__td"><a id="voucherClick" class="voucher-discount__link">Click Here</a></td>
        </tr>
    <?php } ?>  
    ...
    <tr class="voucher-discount" style="display:none;">
        <th>Discount</th>
        <td>
            <input class="voucher-discount__field" type="text" id="voucher_code" name="voucher_code" placeholder="Enter discount code" />
            <button class="voucher-discount__btn" type="button" id="apply_voucher">Add</button>
        </td>
    </tr>
    <script>
...
function apply_voucher_discount() {
    ...
    if ($discount) {
        ...
        // Apply here — early, before reload
        $dummy_code = 'voucher-discount-custom';
        if (!WC()->cart->has_discount($dummy_code)) {
            WC()->cart->apply_coupon($dummy_code);
        }
...
// Lets apply voucher coupon this way

// 1. The filter that provides virtual coupon data when WC asks for a code
add_filter('woocommerce_get_shop_coupon_data', 'create_virtual_voucher_coupon_data', 999, 2);
function create_virtual_voucher_coupon_data($data, $code) {
    // Only handle our dummy codes
    if (strpos($code, 'voucher-discount-') !== 0) {
    ...
    return array(
        'id'                    => 0,
        'amount'                => $voucher_discount * 100,  // WC expects 15 for 15%
        'date_created'          => null,
        'date_modified'         => null,
        'date_expires'          => null,
        'discount_type'         => 'percent',
        'description'           => 'Custom Voucher Discount',
        'usage_count'           => 0,
        'individual_use'        => false,  // allow stacking if needed
        'product_ids'           => array(),
        ...
        'used_by'               => array(),
        'virtual'               => true,  // this is valid here
    );
}

// 2. Apply the dummy code string (in the right place)
add_action('woocommerce_cart_calculate_fees', 'apply_virtual_voucher_dummy_code', 20);
function apply_virtual_voucher_dummy_code($cart) {
    $voucher_discount = WC()->session->get('voucher_discount');
    if (!$voucher_discount || !is_numeric($voucher_discount) || $voucher_discount <= 0) {
        return;
    }

    // Use a fixed dummy code (simpler, no uniqid needed)
    $dummy_code = 'voucher-discount-custom';

    // Apply only if not already present
    if (!$cart->has_discount($dummy_code)) {
        $applied = $cart->apply_coupon($dummy_code);

        if (!$applied) {
            wc_add_notice('Could not apply internal voucher discount (validation failed).', 'error');
        }
    }
}
...
// Convert negative 'Discount Code' fee to a coupon line item in the order object
// This runs server-side during order creation, after cart-to-order transfer
add_action('woocommerce_checkout_create_order', 'convert_voucher_fee_to_coupon_line', 20, 2);
function convert_voucher_fee_to_coupon_line($order, $data) {
    $fee_to_convert = null;
    $discount_amount = 0;
    $discount_tax = 0;

    // Loop through fees to find our negative Discount Code fee
    foreach ($order->get_fees() as $fee_item) {
        if ($fee_item->get_name() === 'Discount Code' && $fee_item->get_total() < 0) {
            $fee_to_convert = $fee_item;
            $discount_amount = abs($fee_item->get_total());      // e.g. 23.61 for -23.61
            $discount_tax   = abs($fee_item->get_total_tax());   // if taxable
            break;
        }
    }

    if (!$fee_to_convert) {
        return; // No matching fee → skip
    }

    // Remove the fee from the order (prevents duplicate)
    $order->remove_item($fee_to_convert->get_id());

    // Create and add a coupon line item
    ...
    // Recalculate taxes and totals to keep everything consistent
    $order->calculate_taxes();
    $order->calculate_totals(false); // false = don't trigger hooks again
}
// Convert negative 'Discount Code' fee to a coupon line item in the order object



// Add this code so that the discount label is displayed correctly

add_filter('woocommerce_cart_totals_coupon_label', 'custom_voucher_label', 10, 2);
function custom_voucher_label($label, $coupon) {
    if (strpos($coupon->get_code(), 'voucher-discount-') === 0) {
        return sprintf(__('Discount Code (%s%%)', 'woocommerce'), $coupon->get_amount());
    }
    return $label;
}
...
// Lets remove the default 'Have a coupon? Click here to enter your code' which appears at the top of the checkout page

add_action( 'init', 'remove_checkout_coupon_form' );
