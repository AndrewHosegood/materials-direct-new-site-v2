...
function calculate_product_price($product_id, $width, $length, $qty, $discount_rate = 0, $sheets_required = 1) {

   if (!is_numeric($product_id) || !is_numeric($width) || !is_numeric($length) || !is_numeric($qty) || !is_numeric($discount_rate) || !is_numeric($sheets_required)) {
    ...
    $sheets_required = intval($sheets_required);
    ...
    $stock_quantity = $product->get_stock_quantity() !== null ? $product->get_stock_quantity() : 0;

    // old logic for out of stock items
    // if ($stock_quantity <= 0) {
    //     $discount_rate = max($discount_rate, 0.05);
    // }

    // new logic for partial backorder
    if ($sheets_required > $stock_quantity) {
    ...
    if (defined('WP_DEBUG') && WP_DEBUG) {
        error_log("Debug [Product ID: $product_id, Width: $width mm, Length: $length mm, Qty: $qty, Discount: $discount_rate, Sheets Required: $sheets_required]:");
...
function calculate_secure_price() {
    ...
    if ($is_product_single) {
        ...
        wp_send_json_success([
            ...
            'stock_quantity' => $product->get_stock_quantity() !== null ? $product->get_stock_quantity() : 0
            'parts_per_sheet' => 1 // Single products have 1 part per sheet
        ]);
        return;
    }
    ...
    $total_price = calculate_product_price($product_id, $width, $length, $qty, $discount_rate, $sheets_required);
...
if ($is_product_single) {
    ...
    // Apply 5% discount and 35-day delivery for backordered items
    if ($sheets_required > $stock_quantity) {
        $discount_rate = max($discount_rate, 0.05); // Ensure 5% discount for backorder
    }
    ...
    $delivery_time = isset($discount_labels[(string)$discount_rate]) ? $discount_labels[(string)$discount_rate] : '35 Days (working days) (5% Discount)';
    $shipments = date('d/m/Y', strtotime($discount_rate == 0 ? '+1 days' : ($discount_rate == 0.015 ? '+2 days' : ($discount_rate == 0.02 ? '+5 days' : ($discount_rate == 0.025 ? '+7 days' : ($discount_rate == 0.03 ? '+12 days' : ($discount_rate == 0.035 ? '+14 days' : ($discount_rate == 0.04 ? '+30 days' : '+35 days')))))));
