...
function calculate_secure_price() {
    ...
    if ($is_backorder) {
        ...
        $total_price -= $backorder_discount_amount;
        $per_part_price = $total_price / $qty;
...
if ($is_product_single) {
    ...
    if (defined('WP_DEBUG') && WP_DEBUG) {
        error_log("add_custom_price_cart_item_data_secure: POST data received");
        error_log("  custom_width: " . (isset($_POST['custom_width']) ? $_POST['custom_width'] : 'not set'));
        error_log("  custom_length: " . (isset($_POST['custom_length']) ? $_POST['custom_length'] : 'not set'));
        error_log("  custom_qty: " . (isset($_POST['custom_qty']) ? $_POST['custom_qty'] : 'not set'));
        error_log("  custom_price: " . (isset($_POST['custom_price']) ? $_POST['custom_price'] : 'not set'));
        error_log("  custom_discount_rate: " . (isset($_POST['custom_discount_rate']) ? $_POST['custom_discount_rate'] : 'not set'));
        error_log("  is_backorder: " . (isset($_POST['is_backorder']) ? $_POST['is_backorder'] : 'not set'));
        error_log("  able_to_dispatch: " . (isset($_POST['able_to_dispatch']) ? $_POST['able_to_dispatch'] : 'not set'));
        error_log("  parts_backorder: " . (isset($_POST['parts_backorder']) ? $_POST['parts_backorder'] : 'not set'));
    }
...
function apply_secure_custom_price($cart) {
    ...
    foreach ($cart->get_cart() as $cart_item_key => $cart_item) {
        ...
        if (isset($cart_item['custom_inputs'])) {
            ...
            // Recalculate sheets required to get parts_per_sheet
            $product = wc_get_product($product_id);
            ...
            if (!is_wp_error($total_price)) {
                ...
                if ($is_backorder) {
                    ...
                    $total_price -= $backorder_discount_amount;
                    if (defined('WP_DEBUG') && WP_DEBUG) {
                        error_log("Cart backorder discount: parts_backorder=$parts_backorder, per_part_price=$per_part_price, discount_amount=$backorder_discount_amount, new_total_price=$total_price");
                    }
                }
                if ($sheets_required <= 0) {
                    ...
                    $cart_item['data']->set_price(sprintf("%.2f", $total_price));
                    continue;
                }
                $total_price_2 = $total_price / $sheets_required;
                $cart_item['data']->set_price(sprintf("%.2f", $total_price_2));
            } else {
                if (defined('WP_DEBUG') && WP_DEBUG) {
