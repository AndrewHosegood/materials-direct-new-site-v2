	...
	<h2 class="<?php echo esc_attr( $heading_class ); ?>">
		...
		if ( $display_order_number ) {
			...
			*/
			...
			$order_number_string = __( '[MD Order: #%s]', 'woocommerce' );
			if ( $email_improvements_enabled ) {
				/* translators: %s: Order ID. */
				$order_number_string = __( 'MD Order: #%s', 'woocommerce' );
			}
			echo wp_kses_post( $before . sprintf( $order_number_string . $after . '<br> Order Date: <time datetime="%s">%s</time>', $order->get_order_number(), $order->get_date_created()->format( 'c' ), wc_format_datetime( $order->get_date_created() ) ) );
			// if ( $email_improvements_enabled ) {
			// 	echo '</span>';
			// }
		}
...
<div style="margin-bottom: <?php echo $email_improvements_enabled ? '24px' : '40px'; ?>;">
	...
	<table class="td one font-family <?php echo esc_attr( $order_table_class ); ?>" cellspacing="0" cellpadding="0" border="1" style="width:100%; border:1px solid #ccc; background: #ffffff; border-collapse: collapse; mso-table-lspace:0; mso-table-rspace:0;">
		...
		<thead>
			<tr>
				...
				<th class="td text-align-<?php echo esc_attr( $order_quantity_text_align ); ?>" scope="col">&nbsp;</th>
				<!-- ah remove quantity field on email -->
	...
	<?php if ( $display_section_divider ) : ?>
		<!-- <hr style="border: 0; border-top: 1px solid #1E1E1E; border-top-color: rgba(30, 30, 30, 0.2); margin: 20px 0;"> -->
		 <div style="height:13px; line-height:13px; font-size:13px;">&nbsp;</div>
		 <table role="presentation" width="100%" cellspacing="0" cellpadding="0" border="0" style="border-collapse:collapse;">
			<tr>
				<td style="border-top:1px solid #cccccc;font-size:0;line-height:0;padding:0;background:#f4f4f4;">
					&nbsp;
				</td>
			</tr>
		</table>
		<div style="height:13px; line-height:13px; font-size:13px;">&nbsp;</div>
	<?php endif; ?>
	<table class="td two font-family <?php echo esc_attr( $order_table_class ); ?>" cellspacing="0" cellpadding="0" border="1" style="width:100%; border:1px solid #ccc; background: #ffffff; border-collapse:collapse; mso-table-lspace:0; mso-table-rspace:0;">
	...
	<?php if ( $order->get_customer_note() && $email_improvements_enabled ) { ?>
		<?php if ( $display_section_divider ) : ?>
			<hr style="border: 0; border-top: 1px solid #1E1E1E; border-top-color: rgba(30, 30, 30, 0.2); margin: 20px 0;">
		<?php endif; ?>
		<table class="td three font-family <?php echo esc_attr( $order_table_class ); ?>" cellspacing="0" cellpadding="6" style="width: 100%;" border="1" role="presentation">
